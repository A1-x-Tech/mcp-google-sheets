...
(`io.github.A1-x-Tech/mcp-google-sheets`). Check before publishing — all three must
...
The npm package is scoped (`@a1-x-tech/mcp-google-sheets`), so `publishConfig.access`
is set to `public` — without it `npm publish` on a scope defaults to private and fails
on a free org.
...
2. Point it at `https://github.com/A1-x-Tech/mcp-google-sheets`. LobeHub pulls the README,
   tool list and install config (`npx -y @a1-x-tech/mcp-google-sheets`) automatically.
