 ...
 * spreadsheet data, tool arguments, prompts and the names or values of
 ...
 * `tool_call` per invocation. `startup_failed` still means "config unusable at
 * load" (a malformed setup such as a partial OAuth triple); it is kept
 * unchanged so the funnel stays comparable across this line of servers.
 * `unconfigured_start` is its live counterpart — the server survives missing
 * credentials and completes the handshake, so this is the first event that can
 * carry the client's name for an install with no credentials yet.
 */
