...
/** Default Google Sheets API host. */
export const DEFAULT_BASE = "https://sheets.googleapis.com";
...
export const MISSING_CREDENTIALS_MESSAGE =
  ...
  "GOOGLE_SHEETS_ACCESS_TOKEN with a short-lived access token. " +
...
export function hasCredentials(config: GoogleSheetsConfig): boolean {
 ...
 *
 *   GOOGLE_SHEETS_CLIENT_ID      OAuth2 client id      (refresh flow, recommended)
 *   GOOGLE_SHEETS_CLIENT_SECRET  OAuth2 client secret  (refresh flow)
 *   GOOGLE_SHEETS_REFRESH_TOKEN  OAuth2 refresh token  (refresh flow)
 *   GOOGLE_SHEETS_ACCESS_TOKEN   static access token (alternative; expires in ~1h)
 *   GOOGLE_SHEETS_API_BASE       Sheets API root override (default https://sheets.googleapis.com)
 *   GOOGLE_SHEETS_TIMEOUT_MS     per-request timeout (default 60000)
 *   GOOGLE_SHEETS_MAX_RETRIES    retries on transient errors (default 3)
 */
export function loadConfig(): GoogleSheetsConfig {
  ...
  if (oauthProvided > 0 && oauthProvided < 3) {
    throw new ConfigError(
      "GOOGLE_SHEETS_CLIENT_ID, GOOGLE_SHEETS_CLIENT_SECRET and GOOGLE_SHEETS_REFRESH_TOKEN must be set together (OAuth2 refresh flow).",
      "incomplete_oauth_config",
    );
  }

  const timeoutMs = Number(process.env.GOOGLE_SHEETS_TIMEOUT_MS);
  const maxRetries = Number(process.env.GOOGLE_SHEETS_MAX_RETRIES);
  ...
  return {
    ...
    refreshToken,
    accessToken,
    apiBase: process.env.GOOGLE_SHEETS_API_BASE || DEFAULT_BASE,
