 ...
 * that leaves the user a dead server and no reason. It is a survivable state:
 * the server starts degraded and the client raises CredentialsError on the
 * first call instead (pinned in client.test.ts). Reverting this would restore
 * that dead end.
 */
test("no credentials at all is not an error — the config loads with empty fields", () => {
  withEnv({}, () => {
    ...
    assert.equal(config.apiBase, "https://sheets.googleapis.com");
    assert.equal(hasCredentials(config), false);
  });
});

test("a partial OAuth triple reports incomplete_oauth_config", () => {
  assert.equal(reasonOf({ GOOGLE_SHEETS_CLIENT_ID: "id" }), "incomplete_oauth_config");
  assert.equal(
    reasonOf({ GOOGLE_SHEETS_CLIENT_ID: "id", GOOGLE_SHEETS_CLIENT_SECRET: "secret" }),
  ...
  assert.equal(
    reasonOf({ GOOGLE_SHEETS_REFRESH_TOKEN: "rt", GOOGLE_SHEETS_ACCESS_TOKEN: "at" }),
    "incomplete_oauth_config",
  );
});
  ...
  withEnv(
    {
      ...
      GOOGLE_SHEETS_REFRESH_TOKEN: "rt",
    },
    () => {
      const config = loadConfig();
...
test("a static access token alone is enough", () => {
  withEnv({ GOOGLE_SHEETS_ACCESS_TOKEN: "at" }, () => {
    const config = loadConfig();
...
test("invalid numeric overrides fall back to the defaults", () => {
  withEnv(
    {
      GOOGLE_SHEETS_ACCESS_TOKEN: "at",
      GOOGLE_SHEETS_TIMEOUT_MS: "not-a-number",
...
test("numeric overrides are honored when valid", () => {
  withEnv(
    { GOOGLE_SHEETS_ACCESS_TOKEN: "at", GOOGLE_SHEETS_TIMEOUT_MS: "1000", GOOGLE_SHEETS_MAX_RETRIES: "0" },
    () => {
      const config = loadConfig();
...
test("the API base override is honored", () => {
  withEnv({ GOOGLE_SHEETS_ACCESS_TOKEN: "at", GOOGLE_SHEETS_API_BASE: "https://example.test" }, () => {
